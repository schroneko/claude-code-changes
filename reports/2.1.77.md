...
Compared with `2.1.76`

## Source-Only Highlights
### Package Files
- CHANGED: `cli.js` (text, 550914 -> 552469 lines)
- CHANGED: `package.json` (text, 33 -> 33 lines)
- CHANGED: `sdk-tools.d.ts` (text, 2847 -> 2832 lines)

### Environment Variables
- ADDED: `CLAUDE_CODE_NEW_INIT`
- ADDED: `CLAUDE_COWORK_MEMORY_EXTRA_GUIDELINES`

### SDK Tools
- CHANGED: `AgentInput` (-resume?)
...
## 2. Slash Commands
- ADDED: `/branch` [official]
- REMOVED: `/fork`

## 3. Public Surface
### Models
- ADDED: `claude-hook-check.txt`
...
- Slash / Workflows (changed)
  - Official: Fixed input deadlock when opening `/mcp` or similar dialogs while the agent is running
  - Official: Renamed `/fork` to `/branch` (`/fork` still works as an alias)
  - Source: Slash command removed: /fork
  - Command example: "Output from a local slash command (e.g. /voice, /cost). Displayed as assistant-style text in the transcript.",
  - Command example: customTitle: S.string().optional().describe("User-set session title via /rename."),
- MCP / Connectors (changed)
  - Official: Fixed Claude Desktop sessions incorrectly using the terminal CLI's configured API key instead of OAuth
  - Official: Fixed input deadlock when opening `/mcp` or similar dialogs while the agent is running
  - Feature example: setOauthTokenFromFd: () => wA6,
  - Feature example: getOauthTokenFromFd: () => sx1,
- Plugins / Marketplace (changed)
  - Official: Fixed `git-subdir` plugins at different subdirectories of the same monorepo commit colliding in the plugin cache
  - Official: Improved `claude plugin validate` to check skill, agent, and command frontmatter plus `hooks/hooks.json`, catching YAML parse errors and schema violations
  - Config example: CreateMarketplaceModelEndpointCommand: J78,
  - Config example: DeleteMarketplaceModelEndpointCommand: L78,
- Agents / Skills (changed)
  - Official: Fixed a race condition where stale-worktree cleanup could delete an agent worktree just resumed from a previous crash
  - Official: Fixed input deadlock when opening `/mcp` or similar dialogs while the agent is running
  - Command example: R.set("User-Agent", "axios/" + A76, !1);
  - Command example: headers: { "Content-Type": "application/json", "User-Agent": uO(), ...K.headers },
- Memory / Context (changed)
  - Official: Fixed auto-updater starting overlapping binary downloads when the slash-command overlay repeatedly opened and closed, accumulating tens of gigabytes of memory
  - Official: Fixed `--resume` silently truncating recent conversation history due to a race between memory-extraction writes and the main transcript
  - Feature example: sampling: c7({ context: OP.optional(), tools: OP.optional() }).optional(),
  - Feature example: context: c7({ arguments: Tw(m1(), m1()).optional() }).optional(),
- Permissions / Sandbox (changed)
  - Official: Added `allowRead` sandbox filesystem setting to re-allow read access within `denyRead` regions
  - Official: Fixed "Always Allow" on compound bash commands (e.g. `cd src && npm test`) saving a single rule for the full string instead of per-subcommand, leading to dead rules and repeated permission prompts
  - Config example: PERMISSION_DENIED: MfA,
  - Config example: PERMISSION_MODES: () => hW,
- Models / Effort (changed)
  - Official: Increased default maximum output token limits for Claude Opus 4.6 to 64k tokens, and the upper bound for Opus 4.6 and Sonnet 4.6 models to 128k tokens
...
- IDE / UI / Voice / Remote (changed)
  - Official: [VSCode] Improved plan preview tab titles to use the plan's heading instead of "Claude's Plan"
  - Official: [VSCode] When option+click doesn't trigger native selection on macOS, the footer now points to the `macOptionClickForcesSelection` setting
  - Feature example: return eA() && N1.clientType !== "claude-vscode";
  - Feature example: theme: jG(["light", "dark"]).optional(),
