...
Compared with `2.0.77`

## Source-Only Highlights
### Package Files
- CHANGED: `cli.js` (text, 489311 -> 489676 lines)
...
- Slash / Workflows (changed)
  - Official: Added support for running skills and slash commands in a forked sub-agent context using `context: fork` in skill frontmatter
  - Official: Added `/teleport` and `/remote-env` slash commands for claude.ai subscribers, allowing them to resume and configure remote sessions
  - Command example: return IJ.createElement(M0, null, IJ.createElement(C, { dimColor: !0 }, "/plan to preview"));
  - Command example: return Y6.createElement(M0, null, Y6.createElement(C, { dimColor: !0 }, "/plan to preview"));
- MCP / Connectors (changed)
  - Official: Fixed security issue where sensitive data (OAuth tokens, API keys, passwords) could be exposed in debug logs
  - Official: Added support for MCP `list_changed` notifications, allowing MCP servers to dynamically update their available tools, prompts, and resources without requiring reconnection
  - Feature example: return h0.oauthTokenFromFd;
  - Feature example: h0.oauthTokenFromFd = A;
- Plugins / Marketplace (changed)
  - Official: Fixed plugin path resolution for file-based marketplace sources
  - Official: Fixed `${CLAUDE_PLUGIN_ROOT}` not being substituted in plugin `allowed-tools` frontmatter, which caused tools to incorrectly require approval
  - Config example: _N6 = "DeleteMarketplaceModelEndpointRequest",
  - Config example: _R6 = "ListMarketplaceModelEndpointsRequest",
- Agents / Skills (changed)
  - Official: Added automatic skill hot-reload - skills created or modified in `~/.claude/skills` or `.claude/skills` are now immediately available without restarting the session
  - Official: Added support for running skills and slash commands in a forked sub-agent context using `context: fork` in skill frontmatter
  - Command example: _.set("User-Agent", "axios/" + CAA, !1);
  - Feature example: class fXQ extends bXQ.Agent {
- Memory / Context (changed)
  - Official: Added support for running skills and slash commands in a forked sub-agent context using `context: fork` in skill frontmatter
  - Official: Added real-time thinking block display in Ctrl+O transcript mode
  - Config example: illegalBy: { msg: C1.message, context: FA.slice(f1 - 100, f1 + 100), mode: C1.mode },
  - Feature example: ? RO4(this._latestRoute.context)
- Permissions / Sandbox (changed)
  - Official: Reduced permission prompts for complex bash commands
  - Official: Fixed PreToolUse hooks to allow `updatedInput` when returning `ask` permission decision, enabling hooks to act as middleware while still requesting user consent
  - Config example: return z0A({ permissionMode: G ?? "default", mainLoopModel: Q, exceeds200kTokens: !1 });
  - Config example: PERMISSION_DENIED: g8B,
- Models / Effort (changed)
  - Official: Fixed AWS Bedrock subagents not inheriting EU/APAC cross-region inference model configuration, causing 403 errors when IAM permissions are scoped to specific regions
  - Official: Fixed subagents sometimes not inheriting the parent's model by default
...
- IDE / UI / Voice / Remote (changed)
  - Official: Fixed Claude in Chrome support for WSL environments
  - Official: Added `/teleport` and `/remote-env` slash commands for claude.ai subscribers, allowing them to resume and configure remote sessions
  - Feature example: return d2() && h0.clientType !== "claude-vscode";
  - Feature example: else if (tO("color") || tO("colors") || tO("color=true") || tO("color=always")) xmA = 1;
