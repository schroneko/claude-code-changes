...
### CLI Options
- ADDED: `claude --enable-auto-mode` (Opt in to auto mode)
- ADDED: `claude --workload <tag>` (Workload tag for billing-header attribution (cc_workload). Process-scoped; set by SDK daemon callers that spawn subprocesses for cron work. (only works with --print))
