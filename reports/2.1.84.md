...
Compared with `2.1.83`

## Source-Only Highlights
### Package Files
- CHANGED: `cli.js` (text, 566009 -> 574929 lines)
- CHANGED: `package.json` (text, 34 -> 34 lines)
- CHANGED: `sdk-tools.d.ts` (text, 2820 -> 2825 lines)

### CLI Options
- ADDED: `claude --task-budget <tokens>` (API-side task budget in tokens (output_config.task_budget))

### Environment Variables
- ADDED: `CLAUDE_AI_ORIGIN`
- ADDED: `CLAUDE_CODE_AGENT_LIST_IN_MESSAGES`
- ADDED: `CLAUDE_CODE_IDLE_THRESHOLD_MINUTES`
- ADDED: `CLAUDE_CODE_IDLE_TOKEN_THRESHOLD`
- ADDED: `CLAUDE_STREAM_IDLE_TIMEOUT_MS`
...
- REMOVED: `/stickers`
- REMOVED: `/tasks`
- REMOVED: `/terminal-setup`
- REMOVED: `/think-back`
- REMOVED: `/thinkback-play`
- REMOVED: `/ultrareview`
- REMOVED: `/upgrade`
- REMOVED: `/usage`
- REMOVED: `/version`
...
## 3. Public Surface
### Models
- ADDED: `claude-code-on-the-web`
- ADDED: `claude-code.cli-no-window`
...
- Slash / Workflows (changed)
  - Official: Added idle-return prompt that nudges users returning after 75+ minutes to `/clear`, reducing unnecessary token re-caching on stale sessions
  - Official: Improved interactive startup by ~30ms by running `setup()` in parallel with slash command and agent loading
  - Source: Slash command removed: /clear
  - Source: Slash command removed: /resume
...
- MCP / Connectors (changed)
  - Official: MCP tool descriptions and server instructions are now capped at 2KB to prevent OpenAPI-generated servers from bloating context
  - Official: MCP servers configured both locally and via claude.ai connectors are now deduplicated — the local config wins
  - Feature example: setOauthTokenFromFd: () => qc8,
  - Feature example: getOauthTokenFromFd: () => Ac8,
- Plugins / Marketplace (changed)
  - Official: Added `allowedChannelPlugins` managed setting for team/enterprise admins to define a channel plugin allowlist
  - Source: Slash command removed: /reload-plugins
  - Config example: _F5 = "MarketplaceModelEndpointSummary",
  - Config example: o$8 = "marketplaceModelEndpoint",
- Agents / Skills (changed)
  - Official: Fixed workflow subagents failing with API 400 when the outer session uses `--json-schema` and the subagent also specifies a schema
  - Official: Improved interactive startup by ~30ms by running `setup()` in parallel with slash command and agent loading
  - Feature example: "User-Agent": this.getUserAgent(),
  - Command example: h.set("User-Agent", "axios/" + H56, !1);
- Memory / Context (changed)
  - Official: Added idle-return prompt that nudges users returning after 75+ minutes to `/clear`, reducing unnecessary token re-caching on stale sessions
  - Official: MCP tool descriptions and server instructions are now capped at 2KB to prevent OpenAPI-generated servers from bloating context
  - Source: Slash command removed: /clear
  - Source: Slash command removed: /context
  - Feature example: 5. Context to Preserve
...
- Permissions / Sandbox (changed)
  - Official: Added `allowedChannelPlugins` managed setting for team/enterprise admins to define a channel plugin allowlist
  - Official: Fixed the "allow Claude to edit its own settings for this session" permission option not sticking for users with `Edit(.claude)` allow rules
  - Config example: cz.PermissionDeniedError = pS6;
  - Config example: (T4K(Y, w), V("Applied original permissions to temp file"));
- Models / Effort (changed)
  - Official: Added `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL_SUPPORTS` env vars to override effort/thinking capability detection for pinned default models for 3p (Bedrock, Vertex, Foundry), and `_MODEL_NAME`/`_DESCRIPTION` to customize the `/model` picker label
  - Source: Slash command removed: /model
  - Source: Slash command removed: /think-back
  - Feature example: "claude-opus-4-20250514": 8192,
  - Feature example: "claude-opus-4-0": 8192,
- IDE / UI / Voice / Remote (changed)
  - Official: Fixed voice push-to-talk: holding the voice key no longer leaks characters into the text input, and transcripts now insert at the correct position
  - Official: Fixed missing background color behind certain emoji in user message bubbles on some terminals
  - Source: Slash command removed: /color
  - Source: Slash command removed: /mobile
  - Source: Slash command removed: /remote-env
  - Feature example: return lA() && V8.clientType !== "claude-vscode";
  - Command example: { key: "chrome", pattern: /Chrome(?:\W+(\d+)\.(\d+)(?:\.(\d+))?)?/ },
