...
現在版または指定版の `claude xxx` command tree を一覧表示:
...
- `cli.js` / `cli-formatted.js.gz` 内の Commander 定義
- `.command(...)`
- `.alias(...)`
- `.description(...)`
- `{ hidden: ... }`

これらから `claude xxx` の command tree を抽出し、top-level / nested / hidden を区別します。
...
- run の `Step Summary` に最新 report の Markdown 本文を出すので、`CLI Commands` / `Slash Commands` の差分も GitHub UI 上でそのまま読めます
...
- `Capability Signals` は分類ベースなので、最終判断は `Package Files` と `CLI Commands` と `Slash Commands` も併せて見るのが前提です
